        ...
        self.selectCredentialsFromSources(sources: credSources) { (keyName) in
            if let keyName = keyName, let selectedCredentials = credSources[keyName] {
                ...
                self.performUserVerification(requireUserVerification: requireUserVerification) { (error) in
                    ...
                    let authenticatorData = AuthenticatorData(rpIdHash: Array(rpId.sha256!), userPresent: (requireUserPresence || requireUserVerification), userVerified: requireUserVerification, signCount: newSignCount, attestedCredentialData: nil, extensions: extensions)
